...
class VKF:
    ...
    def __get_sigmas(self):
        ...
        tmp_mat = self.posalert()
        spr_mat = scipy.linalg.cholesky(tmp_mat).T
        
        x_aug = self.x
        ret[0] = x_aug
        for i in range(self.n_dim):
            ret[i+1] = x_aug + gamma*spr_mat.T[i]
            ret[i+1+self.n_dim] = x_aug - gamma*spr_mat.T[i]
        return ret.T

    def update_inputs(self,inputs,data):
